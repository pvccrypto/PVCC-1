...
void SHA256Transform(void* pstate, void* pinput, const void* pinit)
{
    ...
    for (int i = 0; i < 16; i++)
        ((uint32_t*)data)[i] = BytePVCCerse(((uint32_t*)pinput)[i]);
                ...
                if(pindexBest->nHeight > 0){ // Monday, May 20, 2019 12:00:00 AM
                    ...
                    if (hasPayment) {
                        if (bPayMN == true) {
                            ...
                            pblock->vtx[0].vout[0].nValue = blockReward - (masternodePayment);
...
void FormatHashBuffers(CBlock* pblock, char* pmidstate, char* pdata, char* phash1)
{
    ...
    for (unsigned int i = 0; i < sizeof(tmp)/4; i++)
        ((unsigned int*)&tmp)[i] = BytePVCCerse(((unsigned int*)&tmp)[i]);
