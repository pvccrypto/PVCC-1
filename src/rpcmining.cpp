...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "PVCC is not connected!");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    else
    {
        ...
        for (int i = 0; i < 128/4; i++)
            ((unsigned int*)pdata)[i] = BytePVCCerse(((unsigned int*)pdata)[i]);
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "PVCC is not connected!");
