...
{
    widget->setMaxLength(PVCCAddressValidator::MaxAddressLength);
    widget->setValidator(new PVCCAddressValidator(parent));
...
bool parsePVCCURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("PVCC"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!PVCCUnits::parse(PVCCUnits::PVCC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsePVCCURI(QString uri, SendCoinsRecipient *out)
{
    // Convert rev:// to PVCC:
    //
    ...
    {
        uri.replace(0, 11, "PVCC:");
    }
    QUrl uriInstance(uri);
    return parsePVCCURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PVCC.lnk";
}
...
{
    return GetAutostartDir() / "PVCC.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a PVCC.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=PVCC\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the PVCC app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add PVCC app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("PVCC-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  PVCC-qt [" + tr("command-line options") + "]                     " + "\n";
