...
{
    QString ret = QString("PVCC:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PVCC unit input in lnReqAmount, we generate the URI with PVCC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(PVCCUnits::format(PVCCUnits::PVCC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
