...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", PVCCUnits::PVCC);
    ...
    if (!settings.contains("nAnonymizePVCCAmount"))
        settings.setValue("nAnonymizePVCCAmount", 1000);
    nAnonymizePVCCAmount = settings.value("nAnonymizePVCCAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizePVCCAmount"))
        SoftSetArg("-anonymizePVCCamount", settings.value("nAnonymizePVCCAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizePVCCAmount:
            return QVariant(nAnonymizePVCCAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizePVCCAmount:
            nAnonymizePVCCAmount = value.toInt();
            settings.setValue("nAnonymizePVCCAmount", nAnonymizePVCCAmount);
            emit AnonymizePVCCAmountChanged(nAnonymizePVCCAmount);
            break;
        default:
            break;
        }
