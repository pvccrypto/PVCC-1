...
std::string EncodeBase58Check(const std::vector<unsigned char>& vchIn) {
    ...
    uint256 hash = QuarkHash(vch.begin(), vch.end());
...
bool DecodeBase58Check(const char* psz, std::vector<unsigned char>& vchRet) {
    ...
    uint256 hash = QuarkHash(vchRet.begin(), vchRet.end()-4);
...
namespace {
    class CPVCCAddressVisitor : public boost::static_visitor<bool> {
    private:
        CPVCCAddress *addr;
    public:
        CPVCCAddressVisitor(CPVCCAddress *addrIn) : addr(addrIn) { }
...
bool CPVCCAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CPVCCAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CPVCCAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CPVCCAddressVisitor(this), dest);
}

bool CPVCCAddress::IsValid() const {
...
CTxDestination CPVCCAddress::Get() const {
    if (!IsValid())
...
bool CPVCCAddress::GetKeyID(CKeyID &keyID) const {
...
bool CPVCCAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CPVCCSecret::SetKey(const CKey& vchSecret) {
...
CKey CPVCCSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CPVCCSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CPVCCSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CPVCCSecret::SetString(const std::string& strSecret) {
