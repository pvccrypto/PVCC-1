...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PVCC
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PVCC
    // Mac: ~/Library/Application Support/PVCC
    // Unix: ~/.PVCC
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PVCC";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "PVCC.conf"));
...
void BuildConfigFile()
{
    ...
    fprintf(ConfFile, "port=23003\n");
    fprintf(ConfFile, "rpcport=23004\n");
    fprintf(ConfFile, "rpcconnect=127.0.0.1\n");
    fprintf(ConfFile, "rpcallowip=127.0.0.1\n");
    fprintf(ConfFile, "addnode=45.56.105.176:23003\n");
    fclose(ConfFile);
}
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "PVCCd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong PVCC will not work properly.");
