...
const string strMessageMagic = "PVCC Signed Message:\n";
                               ...
                               bool fBlock, bool fMiner, MapPrevTx& inputsRet, bool& fInvalid) const
{
    ...
    for (unsigned int i = 0; i < vin.size(); i++)
    {
        ...
        {
            // PVCCisit this if/when transaction replacement is implemented and allows
            // adding inputs:
            fInvalid = true;
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    hashRand = QuarkHash(BEGIN(hashRand), END(hashRand));
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes)
                    {
                        ...
                        hashKey = QuarkHash(BEGIN(hashKey), END(hashKey));
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        uint256 hash = QuarkHash(vRecv.begin(), vRecv.begin() + nMessageSize);
